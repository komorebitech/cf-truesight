...
export function SegmentDetailPage() {
  ...
  const { data: segment, isLoading: segmentLoading } = useSegment(id, segmentId);
  ...
  const { data: sizeData, isLoading: sizeLoading } = useSegmentSize(id, segmentId, environment);
  const { data: usersData, isLoading: usersLoading } = useSegmentUsers(id, segmentId, {
    page,
    per_page: perPage,
    environment,
  });
  ...
  const openEditDialog = () => {
    ...
    setEditDefinition(segment.definition);
    setEditError("");
    setShowEdit(true);
  };
  ...
  const handleUpdate = async (e: FormEvent) => {
    ...
    if (!trimmedName) {
      setEditError("Segment name is required");
      return;
    }
  ...
  const handleDelete = () => {
    deleteSegment.mutate(
      { projectId: id!, segmentId: segmentId! },
      { onSuccess: () => navigate(`/projects/${id}/segments`) },
    );
    setShowDeleteConfirm(false);
  };

  const getTimeWindowDisplay = (rule: SegmentDefinition["rules"][0]): string => {
    if (!rule.time_window) return "";
    if (typeof rule.time_window === "string") return rule.time_window;
    if (rule.time_window.type === "ever") return "all time";
    return rule.time_window.value ?? "";
  };
        ...
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            <Button
              variant="ghost"
              size="sm"
              onClick={() => navigate(`/projects/${id}/segments`)}
            >
            ...
            <div className="space-y-2">
              <p className="text-sm text-muted-foreground">
                ...
                <span className="font-medium text-foreground">
                  {segment.definition.operator === "and" ? "all" : "any"}
                </span>{" "}
                of the following rules:
              </p>
              <div className="space-y-2">
                {segment.definition.rules.map((rule, i) => (
                  <motion.div
                    key={i}
                  ...
                  >
                    ...
                    {rule.type === "event" ? (
                      <span>
                        {ACTION_LABELS[rule.action ?? "did"] ?? "Performed"}{" "}
                        ...
                        {getTimeWindowDisplay(rule) && (
                          <>
                            {" "}
                            in{" "}
                            <span className="font-medium">{getTimeWindowDisplay(rule)}</span>
                          </>
                        )}
                      </span>
                    ) : (
                      <span>
                        {rule.source === "event" ? "Event" : "User"} property{" "}
          ...
          <CardContent className="p-0">
            {usersLoading ? (
              ...
              <div className="py-12 text-center text-sm text-muted-foreground">
                No users match this segment definition.
              </div>
            ) : (
              <>
                <Table>
      ...
      <Dialog open={showEdit} onOpenChange={setShowEdit}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>Modify the segment definition and settings.</DialogDescription>
          </DialogHeader>
      ...
      <AlertDialog open={showDeleteConfirm} onOpenChange={setShowDeleteConfirm}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Are you sure you want to delete &quot;{segment.name}&quot;? This action cannot be
              undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
